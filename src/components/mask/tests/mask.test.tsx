...
describe('Mask', () => {
  ...
  test('basic usage', async () => {
    ...
    fireEvent.click(getByTestId('mask').querySelectorAll(`.${classPrefix}-aria-button`)[0]);
  ...
  test('renders with opacity', async () => {
    ...
    expect(getByTestId('thick').style.background).toEqual(`rgba(0, 0, 0, ${opacityRecord['thick']})`);
    expect(getByTestId('1').style.background).toEqual(`rgba(0, 0, 0, 0.8)`);
  });

  test('renders with white color', async () => {
    const { getByTestId } = render(<Mask visible color='white' data-testid='mask' />);

    expect(getByTestId('mask').style.background).toEqual(`rgba(255, 255, 255, ${opacityRecord['default']})`);
  });
