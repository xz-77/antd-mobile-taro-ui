...
import { render, testA11y, screen, fireEvent, sleep, waitFor, act } from 'testing';
import Button from '../';
...
describe('Button', () => {
  ...
  test('loading with custom loadingText and loadingIcon', () => {
    const { getByTestId } = render(
      <Button loading data-testid='btn' loadingText='加载中' loadingIcon={<div>loadingIcon</div>}>
        Loading
      </Button>
    );
  ...
  test('renders with onClick', () => {
    class DefaultButton extends React.Component {
      ...
      render() {
        ...
        return (
          <Button loading={loading} loadingText='加载中' onClick={this.enterLoading}>
            Button
          </Button>
        );
      }
